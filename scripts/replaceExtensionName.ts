...
// 通用的文件读取、替换和写入逻辑
async function readReplaceWrite(
  filePath: string,
...
async function parseI18nRelatedFiles(): Promise<void> {
  ...
  const i18nPromises = i18nFiles.map((file) =>
    readReplaceWrite(path.join(i18nDir, file), [
      [new RegExp(oldExtensionTitle, "g"), newExtensionTitle],
    ])
  );
  ...
  await replaceInFiles(
    srcDir,
    new RegExp("i18n:" + oldExtensionName, "g"),
    "i18n:" + newExtensionName
  );
}
...
async function renameFolder(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Successfully renamed folder: ${oldExtensionName} -> ${newExtensionName}`
    );
  } catch (err) {
    console.error(
      `Failed to rename folder: ${oldExtensionName} -> ${newExtensionName}`,
      err
    );
    throw err;
  }
